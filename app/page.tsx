...
const modules = [
  { title: 'Mentalidad correcta', text: 'Por qué fallan las dietas, qué significa control real y cómo entrar al proceso sin humo ni expectativas falsas.' },
  { title: 'Cómo funciona bajar grasa', text: 'Déficit calórico explicado fácil: energía, calorías, balance y qué sí importa de verdad.' },
  { title: 'Cómo comer en la vida real', text: 'Tortillas, arroz, pan, antojos y comidas normales sin tener que prohibirte todo.' },
  { title: 'Control sin obsesión', text: 'Cómo medir, registrar y ajustar porciones sin volverte loco ni vivir castigado.' },
  { title: 'Manejo de antojos y ansiedad', text: 'Hambre real, impulso, ansiedad, autosabotaje y estrategias prácticas desde un enfoque psicológico.' },
  { title: 'Movimiento y actividad', text: 'Pasos, actividad diaria y ejercicio adaptable sin necesidad de vivir en el gimnasio.' },
  { title: 'Estancamientos y sostenibilidad', text: 'Qué hacer cuando te estancas, cómo ajustar y cómo sostener el proceso sin rebotar.' },
];

const support = [
  { title: '4 horas de asesoría mensual', text: 'Para revisar avances, resolver dudas y ajustar el proceso con más claridad.' },
  { title: 'Asistencia por WhatsApp', text: 'Acompañamiento práctico para que no te quedes atorado con dudas del día a día.' },
  { title: 'Garantía de satisfacción', text: 'Si entras, aplicas el método y no sientes que recibiste valor real, te devolvemos tu dinero bajo las condiciones de la garantía.' },
];
    ...
    <main className="min-h-screen bg-[#F7F6F2] text-[#1F1F1F] font-sans pb-24 md:pb-0">
      ...
      <header className="sticky top-0 z-50 border-b border-black/5 bg-[#F7F6F2]/85 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 md:px-8">
          ...
          <a href="#comprar" className="rounded-full bg-[#1F1F1F] px-5 py-2.5 text-sm font-medium text-white transition hover:scale-[1.03]">
            Entrar
          </a>
        </div>
      </header>
      ...
      <section className="mx-auto grid max-w-6xl items-center gap-10 px-5 pb-14 pt-12 md:grid-cols-[1.05fr_0.95fr] md:px-8 md:pb-24 md:pt-20">
        <div className="fade-up">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <a href="#comprar" className="inline-flex items-center justify-center gap-2 rounded-full bg-[#7A9E7E] px-7 py-4 font-semibold text-white shadow-lg shadow-[#7A9E7E]/20 transition hover:scale-[1.02] hover:bg-[#5F7F63] active:scale-[0.98]">
              Quiero aprender el método <Arrow />
            </a>
            <a href="#video" className="inline-flex items-center justify-center rounded-full border border-black/10 bg-white px-7 py-4 font-semibold text-[#1F1F1F] transition hover:scale-[1.02]">
              Ver explicación
            </a>
          </div>
        ...
        <div className="fade-up relative md:pl-4">
          ...
          <div className="relative rounded-[2rem] border border-black/5 bg-white p-5 shadow-2xl">
            <div className="relative flex aspect-[4/5] flex-col justify-between overflow-hidden rounded-[1.5rem] bg-[#E7E4D8] p-6">
              <img src="/yo.jpg" alt="proceso real" className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="grid items-center gap-8 rounded-[2rem] bg-[#1F1F1F] p-6 text-white shadow-2xl md:grid-cols-[1fr_0.95fr] md:p-10 lg:p-14">
          <div>
            ...
            <p className="mb-6 leading-relaxed text-white/70">
              Aquí te explico quién soy, qué estoy siguiendo, por qué funciona para la vida real, cómo opera la asesoría mensual y cómo aplica la garantía de satisfacción.
            </p>
            <a href="#comprar" className="inline-flex items-center justify-center gap-2 rounded-full bg-white px-6 py-3 font-semibold text-[#1F1F1F] transition hover:scale-[1.02]">
              Ver el curso <Arrow />
            </a>
          </div>
          ...
          <div className="relative w-full overflow-hidden rounded-[1.5rem] border border-white/10 shadow-xl bg-black">
            <iframe
              className="h-[520px] w-full rounded-[1.5rem] md:h-[560px]"
              src="https://www.youtube.com/embed/8KxZomgZvdA"
      ...
      <section id="metodo" className="mx-auto max-w-6xl px-5 py-16 md:px-8 md:py-24">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {learn.map((item) => (
            <div key={item.title} className="rounded-3xl border border-black/5 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-md">
      ...
      <section className="mx-auto grid max-w-6xl gap-5 px-5 py-10 md:grid-cols-2 md:px-8 md:py-16">
        <div className="rounded-[2rem] border border-black/5 bg-white p-7 shadow-sm">
          ...
          {['Quieres bajar grasa sin dejar tortillas, arroz o pan', 'Ya te cansaste de dietas imposibles', 'Quieres algo realista', 'Estás dispuesto a medir, aprender y ajustar'].map((text) => (
        ...
        <div className="rounded-[2rem] border border-black/5 bg-white p-7 shadow-sm">
          ...
          {['Resultados mágicos', 'Promesas falsas', 'Que alguien haga el trabajo por ti', 'Contenido gratis disfrazado'].map((text) => (
            <div className="flex gap-3 border-b border-black/5 py-3 last:border-0" key={text}>
              <XMark />
              <p className="text-[#6B6B6B]">{text}</p>
            </div>
